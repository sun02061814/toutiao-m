  ...
  <van-cell class="comment-item">
    ...
    <div slot="label">
      ...
      <div class="bottom-info">
        ...
        <van-button round class="reply-btn" @click="$emit('onreply',commentItem)"
...
<script>
import { likingAPI, relikingAPI } from "@/api/article";
export default {
  name: "commentItem",
  props: {
    commentItem: {
    ...
    async dianzan() {
      ...
      try {
        if (this.commentItem.is_liking) {
          // 已点赞，取消点赞
          await relikingAPI(this.commentItem.com_id);
          this.commentItem.like_count--
        } else {
          // 未点赞，点击点赞
          await likingAPI(this.commentItem.com_id);
          this.commentItem.like_count++
        }
